...
from core.constants import G2A_API_TOKEN, G2A_API_URL
    ...
    while True:
        ...
        for x in results:
            if x['website'] in ['/api/websites/1', '/api/websites/14']:
                if x['name'] != '' and x['name'] not in stations.keys():
                    stations[x['name']] = x['region_key']

        page += 1
    show_message('info', f"{len(stations)} stations loaded", True)
    return stations
